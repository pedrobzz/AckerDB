...
This is the reproducible HITL release gate for DBZZ's two motivating external
hosts. It tests real installed Codex and Claude Code clients against the same
Streamable HTTP endpoint built only from freshly packed `@dbzz/*` tarballs.
...
The gate uses `codex` and `claude` from `PATH`. It packs all five lockstep DBZZ
...
```sh
export DBZZ_MCP_TOKEN='<ephemeral bearer token>'

codex exec --ignore-user-config --ephemeral --skip-git-repo-check \
  --sandbox read-only --json \
  --config 'mcp_servers.dbzz.url="http://127.0.0.1:3211/mcp"' \
  --config 'mcp_servers.dbzz.bearer_token_env_var="DBZZ_MCP_TOKEN"' \
  --config 'mcp_servers.dbzz.default_tools_approval_mode="approve"' \
  'Call the dbzz MCP tool.'

claude 'Call the dbzz MCP tool.' --print --no-session-persistence \
  --output-format stream-json --verbose --permission-mode bypassPermissions \
  --strict-mcp-config --mcp-config \
  '{"mcpServers":{"dbzz":{"type":"http","url":"http://127.0.0.1:3211/mcp","headers":{"Authorization":"Bearer ${DBZZ_MCP_TOKEN}"}}}}'
```
...
traffic without replacing the packaged DBZZ endpoint. It commits the scope and
...
The release-candidate run on 2026-07-17 passed against packed `@dbzz/*` 0.3.0:
...
Real MCP hosts passed against packed @dbzz/* 0.3.0; credentials remained environment-backed and ephemeral.
```
...
- Codex 0.144.0 reports DBZZ's HTTP 403 as `Insufficient scope` and HTTP 401 as
