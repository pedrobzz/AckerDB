...
AckerDB exposes full-text search only on explicitly opted-in application tables,
...
by AckerDB-generated SQLite triggers; applications never manage a second table.
...
`unicode61`. AckerDB quotes each resulting token separately and relies on FTS5's
