...
DBzz V1 stores fixed-dimensional Float32 vectors and supports only exact
cosine, L2, and dot-distance nearest search. SQLite evaluates typed predicates
and uses ordinary indexes first; DBzz streams only eligible primary keys and
...
SQLite and would make DBzz own a cross-platform native SQLite release matrix.
