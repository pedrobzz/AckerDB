...
`acker dev` applies shape-safe changes and clean optimistic changes on every
...
is persisted, and restarting `acker dev` asks again. Keep editing freely: a
...
interactive `acker dev` never runs them unasked. Each refused start asks
...
by deleting its files, or `acker reset`. Production `acker start` and
...
`acker reset` remains the dev escape hatch: it deletes the local database
directory, and the next start initializes fresh (a fresh database stamps the
whole chain as vacuously applied).

`acker generate` never asks for consent — invoking it is the consent — but it
