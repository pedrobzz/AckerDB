...
async function prepare(
  ...
  const response = await fetch(`${value.base}/api/realtime/prepare`, {
    method: "POST",
...
async function offer(value: Fixture, forwarded: string): Promise<string> {
  ...
  const response = await fetch(`${value.base}/api/realtime`, {
    method: "POST",
    headers: { "x-forwarded-for": forwarded },
    body: encode({
      v: PROTOCOL_VERSION,
...
function sessionRequest(
  ...
  return fetch(`${value.base}/api/realtime/${sessionId}`, {
    method,
    headers: { "x-forwarded-for": forwarded },
    ...(method === "PATCH"
      ? {
