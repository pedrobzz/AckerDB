...
} from "../../src/realtime/definition.ts";
...
beforeEach(() => {
  ...
  runtime = new Runtime(
    ...
    {
      realtimeEngine: testRealtimeEngine(
...
describe("realtime HTTP signaling", () => {
  ...
  test("exports bounded setup, recovery, path, media, and cleanup telemetry", async () => {
    ...
    await expectMetric((metric) =>
      metric.name === "runtime.realtime_setup_stage_outcomes" &&
      metric.labels.operation === "realtime" &&
      metric.labels.stage === "auth" &&
      metric.labels.outcome === "ok" &&
      metric.value === 1
    );
    await expectMetric((metric) =>
