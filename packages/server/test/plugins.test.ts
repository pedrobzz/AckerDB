...
describe("definePlugin", () => {
  ...
  test("recognizes descriptors from another compatible package copy", async () => {
    const copySpecifier = "../src/plugins.ts?compatible-package-copy";
    const copy = await import(copySpecifier) as typeof import("../src/plugins.ts");
    const schema = defineSchema({});
  ...
  test("allows one definition identity to reuse one canonical schema across factories", async () => {
    const copySpecifier = "../src/plugins.ts?compatible-schema-factory-copy";
    const copy = await import(copySpecifier) as typeof import("../src/plugins.ts");
