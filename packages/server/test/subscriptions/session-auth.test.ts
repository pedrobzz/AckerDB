...
function auth(attemptId: number, credential: Credential): ClientAuthMessage {
  return { t: "auth", attemptId, credential };
}

function query(id: number): QueryMessage {
  return { t: "q", id, ref: "messages.list", args: {} };
}
...
describe("Session Protocol-2 ownership", () => {
  test("requires hello first and forwards a structured terminal outcome", async () => {
    ...
    // "hello must be the first frame" is the handshake parser's own rule now,
    // so an ungreeted frame is refused at decode like any other frame the
    // surface does not admit — and the session still terminates with the
    // structured outcome the sink forwards.
    await expect(handle(session, query(1))).rejects.toMatchObject({
      code: "malformed",
      message: "the first client frame must be a hello",
    });
    await session.close();
    ...
    expect(sink.controls).toEqual([
      {
        ...
        t: "err",
        id: null,
        outcome: { code: "malformed", retryable: false, message: "the first client frame must be a hello" },
      },
    ]);
  ...
  test("owns anonymous hello and dispatches every non-auth frame with one epoch context", async () => {
    ...
    await handle(session, { t: "sub", id: 1, ref: "messages.list", args: {} });
    await handle(session, { t: "reset", id: 1, cursor: cursor(0) });
    await handle(session, query(2));
    await handle(session, mutation(3));
    await handle(session, { t: "unsub", id: 1 });
  ...
  test("cancels only the matching in-flight procedure", async () => {
    ...
    await handle(session, { t: "cancel", id: 42 });
    expect(runtime.procedureRequests[0]!.signal?.aborted).toBe(false);
    await handle(session, { t: "cancel", id: 41 });
    await running;
