...
import { defineApp, isApp, type AppSchema } from "../src/app.ts";
import { definePlugin, definePluginContract, pluginQuery } from "../src/plugins.ts";
...
describe("defineApp", () => {
  ...
  test("recognizes manifests from another compatible package copy", async () => {
    const copySpecifier = "../src/app.ts?compatible-package-copy";
    const copy = await import(copySpecifier) as typeof import("../src/app.ts");
    const schema = defineSchema({});
