  ...
  test("conservatively intersects every matching write across a small exhaustive domain", () => {
    ...
    const evaluate = (node: PredicateNode, row: Record<string, unknown>): boolean => {
      switch (node.kind) {
        ...
        // JSON-path nodes contribute no exact constraint and are never
        // generated by this property test.
        case "json":
        case "jsonNull":
        case "jsonIn":
          throw new Error("property-test predicates never contain JSON-path nodes");
      }
    };
