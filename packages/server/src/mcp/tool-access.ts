 ...
 * caller Identity's scope grant. An entry's requirement draws from the one
...
export function isMcpToolAuthorized(
  ...
  // A tool entry is a curation surface: even system authority passes a scoped
  // entry only through an explicit grant, unlike function-level scope
  // requirements, which system bypasses at the choke point.
