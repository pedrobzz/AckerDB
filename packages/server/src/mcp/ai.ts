...
export interface McpAiRuntimeCapability {
  /** The application scope vocabulary; local delegations must draw from it. */
 ...
 * so an interactive user principal grants exactly what it requested. A
 * credential-backed principal is delegated authority itself: its local grant
 * intersects with the credential's — authority never exceeds its source.
 */
...
export function createMcpAiTools(
  ...
  for (const scope of normalized.scopes) {
    if (capability.vocabulary === undefined) {
      throw new TypeError(
        `MCP "${mcp.name}" local scopes name scopes but the application declares no scope vocabulary`,
      );
    }
