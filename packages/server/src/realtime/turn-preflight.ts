...
export async function preflightRealtimeTurn(
  ...
  try {
    ...
    const receiverMessage = message(receiverChannel, PROBE, abort.signal);
    callerChannel.send(PROBE);
    await receiverMessage;
    const callerMessage = message(callerChannel, RESPONSE, abort.signal);
...
function collectCandidates(peer: PortableRTCPeerConnection): {
...
function message(
  channel: PortableRTCDataChannel,
  expected: string,
  signal: AbortSignal,
): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    const received = (event: unknown) => {
      const value = (event as { readonly data?: unknown }).data;
