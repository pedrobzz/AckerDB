...
import { bindMcpTokenContext } from "./mcp-token-context.ts";
...
export class Runtime implements RuntimePort {
  ...
  async mutation(context: SessionRuntimeContext, request: RuntimeRequest<MutationMessage>): Promise<RuntimeMutationResult> {
    ...
    return this.runSessionOperation(context, request, "mutation", message.ref, async (state, requestBytes) => {
      ...
      const result = await this.coordinator.execute({
        ...
        work: (db, writes) => invokeFunction(
          fn,
          this.mcpTokenContext(
  ...
  private executeQuery(
    ...
    return this.submitRead(async (connection) => {
      ...
      try {
        ...
        const value = await invokeFunction(
          fn,
          this.mcpTokenContext({ db, auth: principal }, connection, recorder, null),
          args,
        );
        aborted(signal);
        ...
        const execute = async (): Promise<T> => {
          ...
          const result = await this.coordinator.execute({
            ...
            ...(this.telemetry.enabled
              ...
              : {}),
            signal,
            work: (db, writes) => work(this.mcpTokenContext(
              { db, auth: principal },
              this.engine.writer,
              null,
              writes,
  ...
  private mcpTokenContext<T extends { readonly auth: Principal }>(
    context: T,
    connection: Database,
    ...
    return bindMcpTokenContext(frozen, {
      engine: this.engine,
      connection,
      principal: frozen.auth,
      reads,
      writes,
