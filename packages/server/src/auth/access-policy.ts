 ...
 *
 * Typed vocabulary: `defineApp({ scopes: ["notes:read", "notes:write"] as
 * const })` gives `AppScope<typeof app> = "notes:read" | "notes:write"`.
 * Generated server modules instantiate the function builders as
 * `QueryBuilder<S, Caps, Jobs, AppScope<App>>`, so a requirement naming an
 * undeclared scope is a compile error.
 */
