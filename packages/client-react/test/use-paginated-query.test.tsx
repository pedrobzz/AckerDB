...
const logs = { $ref: "logs.list" } as QueryRef<
  LogArgs,
  AckerDBQueryPage<string>,
  LogsGone
>;
...
describe("usePaginatedQuery", () => {
  ...
  test("a page-one boundary shift resubscribes the pages behind it", async () => {
    ...
    // A write lands inside page one: its window shifts and ends earlier.
    await deliverPage(harness, first.id, 2n, { items: ["z", "a"], nextCursor: "c1'" });
    // The old page two no longer starts at page one's boundary: it is
    // replaced, and the flattened window truncates to the proven prefix.
  ...
  test("a shrunken window drops pages past the new end", async () => {
    const harness = createHarness(APP);
    const container = mountPoint();
    const root = createRoot(container);
  ...
  test("losing the connection marks the whole window stale", async () => {
    ...
    // Exhaustion is data-derived and survives staleness.
