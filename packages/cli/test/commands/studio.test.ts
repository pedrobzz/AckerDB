...
function appWithStudio(launcherSource: string, config?: Record<string, unknown>): string {
  ...
  writeFileSync(join(packageDir, "package.json"), JSON.stringify({
    ...
    version: "0.0.0-test",
    type: "module",
    exports: { ".": "./src/launcher/server.ts" },
  }));
  mkdirSync(join(packageDir, "src", "launcher"), { recursive: true });
  writeFileSync(join(packageDir, "src", "launcher", "server.ts"), launcherSource);
  return appDir;
}
...
describe("resolveStudioEntry", () => {
  test("resolves the package from the app's node_modules", () => {
    ...
    expect(resolveStudioEntry(appDir)).toEndWith(
      join("node_modules", "@ackerdb", "studio", "src", "launcher", "server.ts"),
    );
  });
...
test("`acker studio` resolves the launcher and passes the config-derived target", async () => {
  const appDir = appWithStudio(
    `export function startStudio(options) {
      ...
      return { url: "http://127.0.0.1:" + options.port + "/_studio/", stop() {} };
    }\n`,
    { port: 4997 },
  );
  ...
  expect(buffer).toContain(
    "[ackerdb] Studio serving at http://127.0.0.1:4681/_studio/ — proxying to http://127.0.0.1:4997",
  );
});
