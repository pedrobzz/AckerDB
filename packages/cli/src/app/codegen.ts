/**
 * Codegen: turns the entrypoint and definition roots into
 ...
 * entrypoint path, so it is written before anything imports user code — a fresh
 * project whose definition modules import `_generated/server.ts` codegens in
 * one pass. Only `types.ts`/`api.ts` need the entrypoint loaded; definition
...
function serverTs(config: AppConfig, modules: ModuleFile[]): string {
  const appImport = relImport(config.generatedDir, config.entrypoint);
  const { imports: jobImports, root: jobsRoot } =
    moduleTree(modules, config.generatedDir, "dm_");
...
export async function runCodegen(config: AppConfig): Promise<CodegenResult> {
  ...
  const modules = listDefinitionModules(config);
  // server.ts first: it needs only the entrypoint path and definition file list
  // (type-only imports), and writing it before importing user code is what
  // makes fresh projects codegen in one pass.
  emit("server.ts", serverTs(config, modules));
