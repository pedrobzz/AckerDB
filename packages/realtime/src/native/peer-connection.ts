...
export class NativeTrackOwner {
  private readonly tracks = new Map<
    bigint,
    { readonly track: ServerMediaStreamTrack; readonly release: () => void }
  >();
  ...
  wrapTrack(
    ...
    if (entry === undefined) {
      ...
      const wrapped = new ServerMediaStreamTrack(track, this, scope);
      entry = { track: wrapped, release };
...
class NativeRtpOwner {
  ...
  sender(native: NativeRtpSenderBinding): ServerRtpSender {
    return this.wrap(this.senders, native, (value) =>
      new ServerRtpSender(value, this)
    );
  }

  receiver(native: NativeRtpReceiverBinding): ServerRtpReceiver {
    return this.wrap(this.receivers, native, (value) =>
      new ServerRtpReceiver(value, this)
    );
  }

  transceiver(native: NativeRtpTransceiverBinding): ServerRtpTransceiver {
    return this.wrap(this.transceivers, native, (value) =>
      new ServerRtpTransceiver(value, this)
    );
  }
  ...
  private wrap<Native extends { readonly identity: bigint }, Wrapped>(
    cache: Map<bigint, Wrapped>,
    native: Native,
    create: (native: Native) => Wrapped,
  ): Wrapped {
    let wrapped = cache.get(native.identity);
    if (wrapped === undefined) {
      wrapped = create(native);
      cache.set(native.identity, wrapped);
    }
    return wrapped;
  }
}
  ...
  async setParameters(
    ...
    try {
      this.native.setParameters(
        structuredClone(parameters) as NativeRtpParametersBinding,
      );
    } catch (cause) {
