...
const STAFF_TOKEN = process.env.DBZZ_DEMO_STAFF_TOKEN ?? "savoria-demo-staff";
const MCP_PATH = "/mcp";
...
class McpHarness {
  ...
  static async start(): Promise<McpHarness> {
...
async function withBackend(
  ...
  const backend = await McpHarness.start();
  try {
    await work(backend);
  } finally {
